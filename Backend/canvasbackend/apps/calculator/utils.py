...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        ...
        f"RETURN ONLY THE JSON DATA WITHOUT ANY BACKTICKS, CODE BLOCKS, OR MARKDOWN FORMATTING. "
        f"PROPERLY QUOTE THE KEYS AND VALUES IN THE DICTIONARY FOR EASIER PARSING WITH Python's ast.literal_eval."
        f"If the graph is drawn in the image, return the graph plotted. That graph is interactive and can be zoomed in and out. Update these things in the same canvas."
        f"If any type of graph is drawn in the image give the respective the equation of the graph. "
    )
    ...
    if "```" in response_text:
        ...
        if matches:
            response_text = matches[0]  # Take the first code block content
    ...
    try:
        ...
        try:
            # Fall back to ast.literal_eval for Python list/dict syntax
            answers = ast.literal_eval(response_text)
        except Exception as e:
            print(f"Error in parsing response from Gemini API: {e}")
            # If there's a parsing error, try to extract just the JSON part
            try:
                ...
                if match:
                    ...
                    answers = json.loads(extracted_json)
            except Exception as e2:
                print(f"Secondary parsing attempt failed: {e2}")
    
    print('Returned answer:', answers)
    
    # Ensure 'assign' key exists in all answers
